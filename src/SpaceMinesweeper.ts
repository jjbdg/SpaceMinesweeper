 ...
 * 
 * Editor: Jasmine DeGuzman <deguz033@umn.edu>
 */ 

import * as gfx from 'gophergfx'
import { Vector2 } from 'gophergfx';
...
export class SpaceMinesweeper extends gfx.GfxApp
{
    ...
    constructor()
    {
        ...
        this.shipHit = new gfx.Rectangle();
    ...
    update(deltaTime: number): void 
    {
        ...
        this.stars.children.forEach((star: gfx.Transform2) => {
            const starDirection = gfx.Vector2.inverse(moveDirection);
            // TODO QUESTION
            starDirection.multiplyScalar(star.scale.x * shipSpeed);
            star.translate(starDirection);
        ...
        this.mines.children.forEach((mine: gfx.Transform2) => {
            ...
            const mineDirection = gfx.Vector2.inverse(moveDirection);
            
            mineDirection.multiplyScalar(mineSpeed);
            mine.rotation += mineRotationSpeed;
            mine.position.add(mineDirection);
    ...
    onMouseDown(event: MouseEvent): void 
    {
        ...
        laserInstance.scale.set(this.laser.scale.x, this.laser.scale.y);
    ...
    private spawnMine(): void
    {
        ...
        //mineInstance.rotation *= Math.random();
    ...
    private checkForMineCollisions(): void
    {
        for(let i=0; i < this.mines.children.length; i++)
        {
            for(let j=i+1; j < this.mines.children.length; j++)
            {
                ...
                // ADD YOUR CODE HERE
                //QUESTION, why won't the mine turn red?
